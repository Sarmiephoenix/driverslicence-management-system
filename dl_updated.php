<?php   
				...
				if(isset($_GET['submit'])){
					...
					$sql="select dl_id,dl_status,name,cov,mail_id from dl where dscn='$aad' and dl_id='$dl_id'";
					...
					if (mysqli_num_rows($result) > 0) 
					{
						if($row3[1]==0)
						{
							...
							$sql1="update dl set dl_status=$dl_status,dl_issue_date='$date' where dscn='$aad' and dl_id='$dl_id'";
							if($dl_status==1)
							{
								$date=Date("Y-m-d",mktime(0,0,0,date("m"),date("d"),date("Y")))."<br>";
								$sql1="update dl set dl_status=$dl_status,dl_issue_date='$date' where dscn='$aad' and dl_id='$dl_id'";
							...
							$sql7="insert into license(dscn,name,license_no,cov,license_issue_date,license_expiry_date,mail_id) values ('$aad','$name','$lno','$cov','$date','$exp','$mail_id')";
